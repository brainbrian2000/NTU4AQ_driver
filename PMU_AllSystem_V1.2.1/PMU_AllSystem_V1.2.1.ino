...
uint8_t MQTT_Data[PayloadLength] = {
  ...
  (byte)'l', (byte)'a', (byte)'t', (byte)'=',                                   // Message:lat=121.123456 @ i171
  (byte)'1', (byte)'2', (byte)'1', (byte)'.', (byte)'1', (byte)'2', (byte)'3', (byte)'4', (byte)'5', (byte)'6', (byte)'|',

  (byte)'l', (byte)'o', (byte)'n', (byte)'=',                                   // Message:lon=25.123456 @ i186
...
struct MaxSize {
  ...
  uint8_t lat;   // 數據長度 10, lat=121.123456
  uint8_t lon;    // 數據長度 8, lon=25.123456
...
MaxSize pSize = {
  ...
  .lat = 10,   // 數據長度 10, lat=121.123456
  .lon = 9,    // 數據長度 8, lon=25.123456
...
struct PayloadIndex {
  ...
  uint16_t lat;    // 數據長度 10, lat=121.123456
  uint16_t lon;    // 數據長度 8, lon=25.123456
...
PayloadIndex pIndex = {
  ...
  .lat = 169,    // 數據長度 10, lat=121.123456
  .lon = 184,    // 數據長度 8, lon=25.123456
...
//    NBIOT.openNetwork("35.194.169.153", 1883);
    ...
    if (PMU.state == PMU.SLAVER)     Serial.println("[I2C] I2C->slaver");
    if (PMU.state == PMU.MASTER)     Serial.println("[I2C] I2C->master");
    delay(500);
  }
